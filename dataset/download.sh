...
dataset_dir=/expanse/lustre/scratch/anargund/temp_project/data
...
download_set () {
	...
	do
		s=${arr[$i]}
		m=${max[$i]}
        if test -f "$s.tar"; then
            echo " $s done, skipping"
            continue
        fi
        ...
        cat $s.tar.gz.part?? > $s.tar.gz 
        unpigz -p 8 $s.tar.gz
        
        # remove compressed data gz data
        rm -rf $s.tar.gz*
